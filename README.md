...
All skills live in a single canonical `skills/` tree in this repo and are copied to each agent's skill directory. Shared skills (`adversarial-review`, `frontend-design`, `handoff`, `improve-codebase-architecture`, `notion-knowledge-capture`, `notion-read`, `programming`, `sql-read`) deploy to both agents; Codex additionally gets `generate-sprite-sheets`, `gh-ci`, `gh-pr-body`, `gh-comments`, `linear-claim-work`, and `ultragoal`, while Claude additionally gets `gh-address-comments`, `gh-fix-ci`, and `gh-manage-pr`.

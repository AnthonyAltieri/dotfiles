...
- **Child model:** `gpt-6-sol` with `xhigh` reasoning unless the user names another model. Set both explicitly in `create_thread` for every child; do not inherit this thread's model or local Codex defaults.

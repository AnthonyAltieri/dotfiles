...
| --- | --- |
| Every architecture discovery or design report | [Architecture language](references/language.md) |
| Classifying dependencies, seams, adapters, or test migration for a candidate | [Deepening](references/deepening.md) |
| The user asks to compare alternative interfaces for a selected candidate | [Interface design](references/interface-design.md) |
| The user explicitly authorizes creating or updating `CONTEXT.md` | [CONTEXT.md format](references/context-format.md) |
| The user explicitly authorizes recording an architecture decision | [ADR format](references/adr-format.md) |
...
Use project domain terms from `CONTEXT.md` and architecture terms from the language reference. Surface an ADR-conflicting candidate only when observed friction justifies reconsideration, and name the conflict. Ask which candidate the user wants to explore before designing interfaces or implementing code.

### 4. Explore a selected candidate

Clarify constraints, dependencies, invariants, error modes, ordering, and the tests that should survive the refactor. Load the deepening reference when seam or dependency strategy matters. If the user requests alternatives, follow the interface-design reference and compare designs before recommending one.

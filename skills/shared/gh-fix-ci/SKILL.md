...
Use the `gh` CLI to analyze CI status for the current branch PR. The active Nix profile puts the Rust helper commands on `PATH`, so call them directly.
...
2. Identify failed checks and collect logs.
   - Preferred quick path: `inspect-pr-checks --repo "." --pr "<number-or-url>"` (add `--json` for structured summaries).
   - For large raw logs, shrink them locally: `gh run view <run_id> --log-failed | classify-ci-log`. Pull the exact failing job section back into the prompt only when needed.
   - Manual deep dive:
...
3. Handle external CI providers.
   ...
   - Use CircleCI tooling when available in the environment; otherwise report the details URL and classify it as out-of-scope for direct retrieval.
...
6. Correlate with PR changes.
   - Parse the exact error and file/line, and determine whether this PR introduced the failure.
   - Check for pre-existing patterns: `gh run list --branch=<baseRefName> --limit=5`
7. Recommend next action per failure.
   ...
   - Code issue: explain root cause and propose a concrete fix
...
10. Report the CI result: root cause, focused fix, verification, and remaining external checks. Update the PR title or description only when the user explicitly requests that separate operation.

## Output Format

1. Status Overview — passed / failed / skipped checks
2. Failure Analysis — error snippet, root cause, PR correlation
3. Code Examples — key before/after snippets grouped by failure type
4. Summary Table — check, type, error, root cause, resolution
...
Fast inspection of failing checks and extraction of actionable log snippets.
...
Classifies raw CI log text into `build`, `test`, `lint`, `config`, or `environment`, and emits compact JSON snippets around the highest-signal failures. Also accepts a saved log path: `classify-ci-log /tmp/failed.log`.
...
- Keep GitHub fetching and auth on `gh`; the helpers only process saved logs locally.

...
- Use standard tier or higher, even when the user requests light tier for the paired review; report the minimum. Allow Ultragoal to mark completion only when the selected tier's required fresh-context reviewers (at least two) inspected the same current snapshot, the consolidated gate is `passed`, no material finding remains unresolved, and normal verifiers pass on that snapshot.

...
Contents: [Thesis](#thesis) · [Evidence](#evidence-calibration) · [Shape](#shape-visible-dataflow) · [Chaining](#prefer-named-stages-to-chaining) · [Topology](#represent-honest-topology) · [Semantics](#preserve-production-semantics) · [Examples](#examples) · [Failures](#failure-patterns)
...
Evidence is directional, not universal. A processing DSL helped 57 non-professional programmers reconstruct workflow structure more correctly than Python/Pandas, but not faster or with less perceived difficulty, and changed more than arrow notation. More locally linear Java API examples generally reduced response time in a 61-participant study with little correctness or rating effect. Meaningful names helped harder expressions while meaningless temporaries sometimes hurt; full descriptive names improved semantic-defect-location time among professional C# and Java developers; explicit reactive dependencies improved comprehension over callback Observer code in one small student study.

Limits matter: a controlled 104-student study found no overall comprehension or perceived-readability advantage for chaining; functional constructs and decomposition are context-dependent; comprehension studies measure different abilities and do not freely generalize to production; experts read nonlinearly. Optimize trustworthy dependency landmarks, order, domain names, effects, and reconstruction cost—not chaining or minimum line count.

#!/bin/sh
# Vendored from herdr's Claude Code integration asset
# (src/integration/assets/claude/herdr-agent-state.sh) so the dotfiles can
# declare the hook in the managed settings.json instead of letting
# `herdr integration install claude` rewrite a Nix-managed file.
# Refresh it from the herdr release when bumping the herdr version.

---
name: observe
description: Query and manage Observe using the Observe CLI. Use when the user wants to access metrics, logs, traces, or other telemetry data about Laurel infrastructure.
---
...
# LAUREL INFRASTRUCTURE - ORGANIZATION-SPECIFIC CONTEXT

The following sections provide Laurel-specific datasets, field mappings, authentication, and query patterns for the LaurelAI workspace.

## Laurel Authentication

For Laurel infrastructure, use the following customer ID when authenticating:

```bash
~/go/bin/observe --customerid 195118325539 --site observeinc.com login <your-email@laurel.ai> --sso
```

Replace `<your-email@laurel.ai>` with your actual Laurel email address.

## Laurel Dataset Catalog

The LaurelAI workspace contains the following datasets:

### Service & Application Monitoring

- **Service Request Logs** (`LaurelAI.Service Request Logs`) - HTTP request logs with status codes, response times, URLs
- **Endpoint Logs** (`LaurelAI.Endpoint Logs`) - API endpoint access logs
- **Desktop Assistant** (`LaurelAI.Desktop Assistant`) - Desktop application logs
- **Frontend** (`LaurelAI.Frontend`) - Browser/mobile frontend observability data

### Kubernetes & Container Infrastructure

- **Kubernetes Entity** (`LaurelAI.Kubernetes Explorer/Kubernetes Entity`) - K8s resources (pods, services, deployments)
- **Kubernetes Logs** (`LaurelAI.Kubernetes Explorer/Kubernetes Logs`) - Container logs
- **Parsed Container Logs** (`LaurelAI.Kubernetes/Parsed Container Logs`) - Structured container logs
- **Prometheus Metrics** (`LaurelAI.Kubernetes Explorer/Prometheus Metrics`) - K8s cluster metrics

### Distributed Tracing

- **Trace** (`LaurelAI.Tracing/Trace`) - Distributed traces
- **Span** (`LaurelAI.Tracing/Span`) - Individual trace spans
- **Service** (`LaurelAI.Tracing/Service`) - Service inventory from traces
- **Service Metrics** (`LaurelAI.Tracing/Service Metrics`) - Service-level metrics

### Cloud Infrastructure

- **AWS** (`LaurelAI.AWS`) - AWS resource inventory and metrics
- **AWS Lambda Logs** (`LaurelAI.AWS/Lambda Logs`) - Lambda function logs
- **AWS GuardDuty** (`LaurelAI.AWS GuardDuty`) - Security findings
- **VPC Flow Logs** (`LaurelAI.AWS/VPC Flow Logs`) - Network traffic logs

### Databases & Data Stores

- **MongoDB Metrics** (`LaurelAI.MongoDB/Metrics`) - MongoDB performance metrics
- **MongoDB Atlas Metrics** (`LaurelAI.MongoDB Atlas/Metrics`) - Atlas-specific metrics
- **MongoDB Atlas Cluster** (`LaurelAI.MongoDB Atlas/Cluster`) - Cluster information

### Usage & Monitoring

- **Monitor Alarms** (`LaurelAI.usage/Monitor Alarms`) - Active alerts and monitors
- **Observe Usage Metrics** (`LaurelAI.usage/Observe Usage Metrics`) - Platform usage
- **Log Ingest Volume** (`LaurelAI.usage/Log Ingest Volume`) - Log ingestion rates

### Authentication & Security

- **Auth0** (`LaurelAI.Auth0`) - Authentication events and logs

### Development & CI/CD

- **GitHub Actions Run** (`LaurelAI.github/Actions Run`) - CI/CD pipeline runs
- **GitHub Pull Requests** (`LaurelAI.github/Pull Requests`) - PR data
- **GitHub Issues** (`LaurelAI.github/Issues`) - Issue tracking

## Laurel-Specific Field Names

Different datasets use different field names in the LaurelAI workspace. Always verify with `limit 1 -x` or `topk 1 -x` first.

### Kubernetes Logs (Laurel)

- `body` - The log message content (NOT `message`)
- `cluster` - Kubernetes cluster name (e.g., "iad-prd-eks", "lhr-prd-eks") - top-level field
- `namespace` - Kubernetes namespace - top-level field
- `pod` - Pod name - top-level field
- `container` - Container name
- `timestamp` - Log timestamp

**Note**: Kubernetes Logs has `cluster`, `namespace`, and `pod` as top-level fields, making it the most reliable dataset for pod enumeration and namespace-based queries (vs. Kubernetes Entity where these fields are buried in nested/stringified JSON).

### Service Request Logs (Laurel)
...
### Prometheus Metrics (Laurel)

- `metric` - The metric name (e.g., "kube_pod_container_status_restarts_total")
- `value` - The metric value
- `labels` - A nested object containing all metric labels
- `timestamp` - Metric timestamp

**Common Prometheus Labels** (accessed via `labels.<label_name>`):

- `labels.k8s_cluster_name` - Kubernetes cluster name (e.g., "iad-prd-eks", "lhr-prd-eks")
- `labels.namespace` - Kubernetes namespace
- `labels.pod` - Pod name
- `labels.container` - Container name
- `labels.k8s_node_name` - Node name

**Example**: To filter by cluster name in Prometheus metrics:

```opal
filter metric = "kube_pod_container_status_restarts_total" | make_col cluster:string(labels.k8s_cluster_name)
```

### Tracing/Service (Laurel)

- `service_name` - Name of the service (NOT `name`)
- `environment` - Environment (prd, stg, dev)
- `service_namespace` - Service namespace
- `language` - Programming language
- `host_type` - Host type (k8s, etc.)

## Laurel Infrastructure Context

### Clusters

Production clusters:

- `iad-prd-eks` - US East (Virginia) production
- `lhr-prd-eks` - Europe (London) production

Staging/Dev clusters:

- `iad-stg-eks` - US East (Virginia) staging

### Common Services

Check the Tracing/Service dataset for a full list:

```bash
~/go/bin/observe query -q 'topk 100' -i 'LaurelAI.Tracing/Service' -r 24h
```

Common services include: time, enrichment-api, payment, etc.

## Laurel-Specific Query Examples

### Investigating Service Errors (Laurel)

```bash
# Find recent 5xx errors by service
~/go/bin/observe query -q 'filter statusCode >= 500 | timechart 5m, count(1), group_by(service, statusCode)' -i 'LaurelAI.Service Request Logs' -r 24h

# Get detailed error logs
~/go/bin/observe query -q 'filter statusCode >= 500 | limit 50' -i 'LaurelAI.Service Request Logs' -r 1h -x
```

### Kubernetes Pod Investigation (Laurel)

```bash
# Count active pods per namespace across clusters
~/go/bin/observe query -q 'filter namespace = "time" | timechart 1h, count_distinct(pod), group_by(cluster)' -i 'LaurelAI.Kubernetes Explorer/Kubernetes Logs' -r 1h

# List distinct pod names in a namespace
~/go/bin/observe query -q 'filter namespace = "enrichment" and cluster = "iad-prd-eks" | timechart 1h, count_distinct(pod)' -i 'LaurelAI.Kubernetes Explorer/Kubernetes Logs' -r 1h

# List pods via Kubernetes Entity (Resource dataset - use 'topk')
~/go/bin/observe query -q 'filter kind = "Pod" and string(identifiers.clusterName) = "iad-prd-eks" | topk 50' -i 'LaurelAI.Kubernetes Explorer/Kubernetes Entity' -r 1h
```

### Pod Restart Analysis (Laurel)

```bash
# Check pod restarts using Prometheus Metrics
~/go/bin/observe query -q 'filter metric = "kube_pod_container_status_restarts_total" and float64(value) > 0 | limit 50' -i 'LaurelAI.Kubernetes Explorer/Prometheus Metrics' -r 24h

# Analyze pod restarts by cluster and namespace
~/go/bin/observe query -q 'filter metric = "kube_pod_container_status_restarts_total" | make_col cluster:string(labels.k8s_cluster_name), namespace:string(labels.namespace), restarts:float64(value) | timechart 48h, max(restarts), group_by(cluster, namespace)' -i 'LaurelAI.Kubernetes Explorer/Prometheus Metrics' -r 48h
```

### Container Log Search (Laurel)

```bash
# Search container logs for error patterns
# IMPORTANT: Use 'body' field, NOT 'message'
~/go/bin/observe query -q 'filter body ~ /error|exception|fail/i | limit 50' -i 'LaurelAI.Kubernetes Explorer/Kubernetes Logs' -r 1h -x

# Search specific namespace logs
~/go/bin/observe query -q 'filter namespace = "enrichment" and body ~ /error/i | limit 30' -i 'LaurelAI.Kubernetes Explorer/Kubernetes Logs' -r 6h -x

# Search logs in staging cluster
~/go/bin/observe query -q 'filter cluster = "iad-stg-eks" and body ~ /error/i | limit 30' -i 'LaurelAI.Kubernetes Explorer/Kubernetes Logs' -r 24h -x
```

### Service Tracing (Laurel)

```bash
# Find a specific service by name
~/go/bin/observe query -q 'filter service_name = "time" | topk 20' -i 'LaurelAI.Tracing/Service' -r 24h -x

# Find slow traces
~/go/bin/observe query -q 'filter duration > 1000000000 | limit 20' -i 'LaurelAI.Tracing/Trace' -r 1h -x
```

### CI/CD Pipeline Analysis (Laurel)

```bash
# Recent GitHub Actions failures (Resource dataset - use 'topk')
~/go/bin/observe query -q 'filter conclusion = "failure" | topk 50' -i 'LaurelAI.github/Actions Run' -r 24h -x

# Find all non-successful runs
~/go/bin/observe query -q 'filter conclusion ~ /(failure|cancelled|timed_out)/ | topk 50' -i 'LaurelAI.github/Actions Run' -r 24h -x
```

### Authentication & Security (Laurel)

```bash
# Failed authentication attempts
~/go/bin/observe query -q 'filter type = "f" | limit 50' -i 'LaurelAI.Auth0' -r 24h -x

# AWS GuardDuty findings (last 7 days)
~/go/bin/observe query -q 'limit 20' -i 'LaurelAI.AWS GuardDuty' -r 168h -x
```

### Database Monitoring (Laurel)

```bash
# MongoDB metrics
~/go/bin/observe query -q 'timechart 1m, avg(value), group_by(metric_name)' -i 'LaurelAI.MongoDB/Metrics' -r 1h

# MongoDB cluster health
~/go/bin/observe query -q 'topk 10' -i 'LaurelAI.MongoDB Atlas/Cluster' -r 1h -x
```

## Laurel-Specific Troubleshooting

### Finding Airflow DAG Errors in Staging

```bash
# Search for Airflow pod errors in staging
~/go/bin/observe query -q 'filter cluster = "iad-stg-eks" and pod ~ /airflow/ and body ~ /"level": "ERROR"|"level": "WARNING"/ | limit 30' -i 'LaurelAI.Kubernetes Explorer/Kubernetes Logs' -r 72h -x

# Get full error details in JSON
~/go/bin/observe query -q 'filter cluster = "iad-stg-eks" and pod ~ /airflow/ and body ~ /"level": "ERROR"/ | limit 3' -i 'LaurelAI.Kubernetes Explorer/Kubernetes Logs' -r 72h --json > airflow_errors.json
```

### Filtering by Cluster in Different Datasets (Laurel)

Each dataset stores cluster information differently:

```bash
# Kubernetes Logs - use 'cluster' field directly
~/go/bin/observe query -q 'filter cluster = "iad-stg-eks" | limit 50' -i 'LaurelAI.Kubernetes Explorer/Kubernetes Logs' -r 24h

# Kubernetes Entity - use identifiers.clusterName
~/go/bin/observe query -q 'filter string(identifiers.clusterName) = "iad-stg-eks" | topk 50' -i 'LaurelAI.Kubernetes Explorer/Kubernetes Entity' -r 24h

# Prometheus Metrics - use labels.k8s_cluster_name
~/go/bin/observe query -q 'filter string(labels.k8s_cluster_name) = "iad-stg-eks" | limit 50' -i 'LaurelAI.Kubernetes Explorer/Prometheus Metrics' -r 24h

# Service Request Logs - use 'cluster' field directly
~/go/bin/observe query -q 'filter cluster = "iad-stg-eks" | limit 50' -i 'LaurelAI.Service Request Logs' -r 24h
```

## Laurel-Specific Pitfalls

### Kubernetes Logs: Use 'body' not 'message'
...
### Kubernetes Entity: data field is a JSON string

The `data` field in Kubernetes Entity is a stringified JSON, NOT a parsed object:

```opal
# WRONG - data is a JSON string, not traversable
filter string(data.metadata.namespace) = "time"

# CORRECT - use Kubernetes Logs instead, which has top-level fields
filter namespace = "time"

# OR use regex matching on the stringified data
filter string(data) ~ /"namespace":"time"/
```

### Resource datasets require 'topk' instead of 'limit'

```opal
# WRONG for Resource datasets (GitHub Actions, Kubernetes Entity)
limit 50

# CORRECT for Resource datasets
topk 50

# Event datasets (logs, traces, metrics) still use 'limit'
```

## Laurel Quick Reference

```bash
# List available Laurel datasets
~/go/bin/observe list dataset | grep -i laurel

# Check Laurel dataset schema
~/go/bin/observe query -q 'limit 1' -i 'LaurelAI.Kubernetes Explorer/Kubernetes Logs' -r 1h -x
```

...
3) Pull any prior context from Notion with the available search/fetch tools so you can update or link existing pages instead of duplicating them.
4) Draft the page with the available Notion page-creation tools using the database schema; include summary, context, source links, and tags/owners.
5) Link from hub pages and related records; update status/owners with the available page-update tools as the source evolves.
...
- Use the available Notion page-creation tools with the correct `data_source_id`; set properties (title, tags, owner, status, dates, relations).

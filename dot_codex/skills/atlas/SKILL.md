...
Use the bootstrap-installed `atlas-cli` helper to control Atlas and inspect local browser data.

## Quick Start

Bootstrap installs `atlas-cli` into `~/.local/bin`, so call it directly.
...
If `atlas-cli` is missing, rerun bootstrap so the installed helper is refreshed in `~/.local/bin`.
